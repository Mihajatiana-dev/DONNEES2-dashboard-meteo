...
def extract_weather_and_air(city: str, api_key: str, date: str) -> bool:
    ...
    try:
        ...
        combined_data = {
            ...
            'date_observation': datetime.utcfromtimestamp(weather_data['dt']).strftime("%Y-%m-%d %H:%M:%S"),
        ...
        # 5. Sauvegarde
        os.makedirs(f"dags/data/raw/{date}", exist_ok=True)
        pd.DataFrame([combined_data]).to_csv(
            f"dags/data/raw/{date}/weather_air_{city}.csv", 
            index=False
        )
